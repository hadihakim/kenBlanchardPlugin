class CourseDetails {
    ...
    static courseRender = () => {
        ...
        this.state.data.lessons.forEach((el) => {
            ...
            let title = ui.createElement("p", lessons[0], el.subtitle, ["course-lesson-subtitle", "headerText-AppTheme", "assetsTitle"], 'course-lesson-subtitle');
            ...
            for (let i = 0; i < el.assets.length; i++) {
                ...
                if (assets.type == "video") {
                    lessonIcon.innerHTML = `<label class="material-icons icon" id="movieIcon">videocam</label> `;
                    assetsIcon.innerHTML = `<label class="material-icons icon" id="movieIcon">videocam</label> `;

                } else if (assets.type == "audio") {
                    lessonIcon.innerHTML = `<label class="material-icons icon" id="podcastIcon">headphones</label> `;
                    assetsIcon.innerHTML = `<label class="material-icons icon" id="podcastIcon">headphones</label> `;

                } else if (assets.type == "article") {
                    lessonIcon.innerHTML = `<label class="material-icons icon" id="articleIcon">article</label> `;
                    assetsIcon.innerHTML = `<label class="material-icons icon" id="articleIcon">article</label> `;
                ...
                ui.createElement("p", assetsDetails, assets.title, ["course-lesson-subtitle", "headerText-AppTheme", "assetsTitle-course"], 'course-lesson-subtitleDetails');
                ui.createElement("div", assetsDetails, `<div class="duration duration-lesson-details">
            ...
            if (el.assets.length < 2) {
                ...
                let span = ui.createElement('span', title, 'expand_less', ["material-icons", "icon", "arrow-up-icon"])
        ...
        duration[0].innerHTML = `<span class="material-icons icon details-icon schedule-icon" id="scheduleIcon"> schedule </span>
                                     <span class="schedule-text bodyText-AppTheme">
                            ${Utilities.timeConvert(this.state.duration)}</span>`;
