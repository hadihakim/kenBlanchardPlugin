...
const templates = () => {
	...
	const seeAllCardsRender = (apiData, container, durationState, callback) => {
		...
		let sectionData = apiData.data.sections.find(({ id }) => id == config.activeSeeAll) || {};
		let assetsInfo = [];
		...
		for (lastIndex; lastIndex < (lastIndex + config.pageSize); lastIndex++) {
			if (config.renderedCard == (config.pageSize * config.page) ||
				...
				topicIdArray.forEach((topicId) => {
					...
					if ((config.filterArr.includes(data.title) || config.filterArr.length === 0) && hasSearch(assetsInfo[lastIndex])) {
						printCard = true;
					}
				});
				if (printCard) {
