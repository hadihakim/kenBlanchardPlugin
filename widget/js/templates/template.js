const templates = () => {
  const forYouRender = (apiData, container, duration) => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      image.style.backgroundImage = `linear-gradient(0deg, rgba(0, 0, 0, 0.32), rgba(0, 0, 0, 0.32)), url('${buildfire.imageLib.cropImage(
        data.meta.image,
  ...
  const recommendedCardRender = (apiData, container, durationState) => {
    ...
    for (let index = 0; index < 6; index++) {
      ...
      image[0].style.backgroundImage = `url('${buildfire.imageLib.cropImage(
        data.meta.image,
        { size: "full_width", aspect: "16:9" }
      )}')`;
