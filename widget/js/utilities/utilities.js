	...
	static bookmark=()=>{
		const addBookmark=(options,type)=>{
			buildfire.bookmarks.add(
				...
				(err, bookmark) => {
				  if (err) return console.error(err);
				  this.showToast(options.type === "video" ? Strings.VIDEO_ADDED_BOOKMARK :options.type === "audio"? Strings.AUDIO_ADDED_BOOKMARK:options.type === "article"?Strings.ARTICLE_ADDED_BOOKMARK:Strings.SHORTCUT_ADDED_BOOKMARK)
		...
		const deletesBookmark=(id,type)=>{
			buildfire.bookmarks.delete(id, () => {
				this.showToast(type === "video" ? Strings.VIDEO_REMOVED_BOOKMARK : type === "audio"? Strings.AUDIO_REMOVED_BOOKMARK: type === "article"? Strings.ARTICLE_REMOVED_BOOKMARK:Strings.SHORTCUT_REMOVED_BOOKMARK)
