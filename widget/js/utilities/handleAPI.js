class HandleAPI {
  ...
  static updateAssetProgress = (id, progress) => {
    UserProfile.state.data.assets[id]
      ? (UserProfile.state.data.assets[id].progress = 50)
      : (UserProfile.state.data.assets[id] = { progress: 50 });
    return new Promise((resolve, reject) => {
      Profiles.updateAssetProgress(id, progress, async (err, res) => {
        if (err) reject(err);
        resolve(res);
      });
    });
  ...
  static setAssetArchiveStatus = (id,status) => {
    return new Promise((resolve, reject) => {
        Profiles.setAssetArchiveStatus(id, status, (err,res)=>{
            if(err) return reject(err);
            resolve(res);
        });
    });
  ...
  static handleFilter = (topics, idx = 0) => {
    ...
    let topicData = this.state.topics.find(({ id }) => id === topics[idx]);
