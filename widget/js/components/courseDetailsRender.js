    ...
    this.state.data.lessons.forEach((el) => {
      ...
      if(el.assets.length === 1) {
        ...
        lessonContainer.addEventListener('click', () => {
          ...
          Navigation.openPageDetails({id:el.assets[0], title:this.state.data.meta.title,fromLocalNotifications: false,pushToHistory:true});
        })
      ...
      for (let i = 0; i < el.assets.length; i++) {
        ...
        assetsDetails.addEventListener("click", ()=> {
          Navigation.openPageDetails({ id:el.assets[i],  title:this.state.data.meta.title,fromLocalNotifications:false,pushToHistory:true});
        })
